...
class Source:
    def __init__(self, source_config: dict) -> None:
        ...
        for key, value in source_config.items():
            if isinstance(value, dict):
                # 处理能量分布（erg=D1）或其他分布
                if 'si' in value and 'sp' in value:  # 确保 si 和 sp 存在
                    if value['si'] and value['sp']:  # 确保 si 和 sp 非空
                        self.si_sp_id += 1
    ...
    def to_file(self, file: FileIO):
        ...
        if self.si_sp_str:  # 如果有 SI/SP 定义，写入文件
            file.write(self.si_sp_str)

    def _parse_si_sp(self, si: list, sp: list):
        # 检查 si 和 sp 的长度是否匹配
        if len(si) != len(sp):
            raise ValueError(f"Length of si ({len(si)}) and sp ({len(sp)}) must match.")

        # 检查 sp 的第一个值是否为零
        if sp[0] != 0:
            raise ValueError("The first value of sp must be zero.")

        # 检查 sp 的其他值是否为非负数
        for weight in sp[1:]:
            if weight < 0:
                raise ValueError("All weights in sp must be non-negative.")

        # 生成 SI 和 SP 卡
        si_sp_str = f'SI{self.si_sp_id} {" ".join(map(str, si))}\n'
        si_sp_str += f'SP{self.si_sp_id} {" ".join(map(str, sp))}\n'
        return si_sp_str
