...
def _build_fallback_phantom() -> np.ndarray:
    """
    当ICRP-110数据不存在时生成合成解剖体模。

    包含基本解剖结构，使用 CT 融合兼容材质码：
      107 = 软组织（体内默认）
       46 = 骨骼（脊柱、骨盆、股骨、肋骨、颅骨）
       81 = 肺/气腔（双侧肺野）

    体模尺寸对应 AM 体模: 254(X) × 127(Y) × 222(Z) 体素
    物理间距: 2.137 × 2.137 × 8.0 mm，Z=0为足底，Z=221为头顶
    """
    print("  [Fallback] 生成合成解剖体模（含脊柱/肺/骨盆/股骨）")
    nx, ny, nz = 254, 127, 222
    phantom = np.zeros((nx, ny, nz), dtype=np.int16)
    ...
    # ── 1. 软组织体（椭圆柱体近似身体轮廓）──────────────────────────
    # (z_lo, z_hi, cx, cy, rx, ry) — 各段身体椭圆截面参数
    segments = [
        ( 5,  14, 127, 56, 20, 12),   # 踝/脚
        (14,  42, 127, 56, 27, 16),   # 小腿
        (42,  62, 127, 57, 30, 18),   # 膝
        (62,  92, 127, 58, 34, 20),   # 大腿
        (92, 118, 127, 58, 43, 27),   # 骨盆/臀
        (118,152, 127, 55, 46, 29),   # 腹部
        (152,185, 127, 52, 49, 30),   # 胸部
        (185,196, 127, 52, 25, 20),   # 颈部
        (196,220, 127, 55, 33, 30),   # 头部
    ]
    for z_lo, z_hi, cx, cy, rx, ry in segments:
        seg = (((x - cx) ** 2 / rx ** 2 + (y - cy) ** 2 / ry ** 2) <= 1.0) & \
              (z >= z_lo) & (z < z_hi)
        phantom[seg] = 107
    ...
    # ── 2. 脊柱（后正中线，全长骨柱）───────────────────────────────
    spine = ((x - 127) ** 2 <= 16) & ((y - 97) ** 2 <= 9) & \
            (z > 15) & (z < 208) & body
    phantom[spine] = 46

    # ── 3. 肋骨（简化为10对水平弧环）───────────────────────────────
    for zr in range(155, 186, 3):
        rib_d = np.sqrt((x - 127) ** 2 + (y - 68) ** 2)
        rib = (rib_d >= 28) & (rib_d <= 32) & (y < 98) & \
              (z >= zr) & (z < zr + 2) & body
        phantom[rib] = 46

    # ── 4. 骨盆（椭圆环形结构）─────────────────────────────────────
    pelvis_d = np.sqrt((x - 127) ** 2 + (y - 68) ** 2)
    pelvis = (pelvis_d >= 32) & (pelvis_d <= 40) & \
             (z >= 82) & (z < 114) & body
    phantom[pelvis] = 46

    # ── 5. 股骨（双侧长骨柱）──────────────────────────────────────
    fem_l = ((x - 100) ** 2 + (y - 62) ** 2 <= 25) & (z >= 16) & (z < 88)
    fem_r = ((x - 154) ** 2 + (y - 62) ** 2 <= 25) & (z >= 16) & (z < 88)
    phantom[fem_l & body] = 46
    phantom[fem_r & body] = 46

    # ── 6. 颅骨轮廓（椭球壳）──────────────────────────────────────
    skull_d = ((x - 127) ** 2 / 30 ** 2 +
               (y - 54) ** 2 / 26 ** 2 +
               (z - 210) ** 2 / 13 ** 2)
    skull = (skull_d <= 1.0) & (skull_d > 0.52) & (z > 197)
    phantom[skull & body] = 46

    # ── 7. 肺（双侧椭球气腔）──────────────────────────────────────
    lung_l = (((x - 100) ** 2 / 27 ** 2 +
                (y -  50) ** 2 / 23 ** 2 +
                (z - 167) ** 2 / 17 ** 2) <= 1.0) & (x < 122)
    lung_r = (((x - 154) ** 2 / 27 ** 2 +
                (y -  50) ** 2 / 23 ** 2 +
                (z - 167) ** 2 / 17 ** 2) <= 1.0) & (x > 132)
    phantom[lung_l & body] = 81
    phantom[lung_r & body] = 81

    bone_n = int(np.sum(phantom == 46))
    lung_n = int(np.sum(phantom == 81))
    soft_n = int(np.sum(phantom == 107))
    print(f"  合成体模: 骨骼={bone_n:,}  肺={lung_n:,}  软组织={soft_n:,}")
    return phantom
