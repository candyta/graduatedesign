...
if __name__ == '__main__':
    ...
    parser.add_argument('--masks', required=True, help='逗号分隔的mask NIfTI路径')
    parser.add_argument('--names', default='',    help='逗号分隔的器官名称')
    ...
    mask_names = [n.strip() for n in args.names.split(',') if n.strip()] \
                 if args.names else [f'Organ{i+1}' for i in range(len(mask_paths))]
